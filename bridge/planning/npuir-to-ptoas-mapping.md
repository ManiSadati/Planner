...
| `hivm.hir.load` | In `dma_copy_kernel`, remains structured through `hivm-mark-disable-load`; `convert-hivm-to-std` rewrites it to `load_gm_to_ubuf_1d_float`. The Flash/qk path also exposes f16 rank-2 implicit-transpose loads before that conversion. | Contiguous loads map directly to low-level VPTO `pto.mte_gm_ub`. The observed 64x64 f16 implicit transpose imports a PTODSL helper that performs GM-to-UB MTE followed by pairwise in-place `vgather2`/`vscatter` swaps. Longer-term performance target is native NDDMA-like PTO support. | After `hivm-mark-disable-load`, before `convert-hivm-to-std`. | confirmed for contiguous 1D; fixed 64x64 f16 transpose specialization matches the non-symmetric qk reference in every currently populated output column | Preserve memory scope, UB placement, logical view, runtime dimensions, element width, padding, and explicit sync ownership. Do not assume DMA strides perform a transpose. | N3, N4, N9, P3, P6, P13 | Other transpose shapes/layouts/datatypes are explicitly rejected. The in-place helper avoids adding UB scratch after memory planning and limits live vector registers. The separate zero-columns-16-63 failure is downstream of this conversion. |

...
KERNEL_OBJ="${KERNEL_OBJ:-${SCRIPT_DIR}/flash_attention_128q_4096kv_d64_kernel.o}"
...
if [[ ! -f "${KERNEL_OBJ}" ]]; then
    ...
    echo "[ERROR] Copy the Flash Attention fatobj here or set KERNEL_OBJ=/path/to/kernel.o." >&2
    exit 1
fi
...
"${BISHENG_BIN}" \
    ...
    -shared -Wl,-soname,libflash_attention_128q_4096kv_d64_kernel.so \
    -L "${ASCEND_HOME_PATH}/lib64" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/libflash_attention_128q_4096kv_d64_kernel.so" \
    "${KERNEL_OBJ}" \
    "${BUILD_DIR}/launch.o" \
...
"${BISHENG_BIN}" \
    ...
    -Wl,-rpath,"${BUILD_DIR}" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/flash_attention_128q_4096kv_d64" \
    -lflash_attention_128q_4096kv_d64_kernel \
...
LD_LIBRARY_PATH="${BUILD_DIR}:${ASCEND_HOME_PATH}/lib64:${LD_LIBRARY_PATH:-}" \
    "${BUILD_DIR}/flash_attention_128q_4096kv_d64"

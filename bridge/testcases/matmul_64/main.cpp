...
int main() {
    ...
    if (!CheckAcl(aclInit(nullptr), "aclInit") ||
        ...
        !CheckAcl(aclrtMallocHost(reinterpret_cast<void **>(&inputAHost), kDataBytes),
                  "aclrtMallocHost(inputA)") ||
        !CheckAcl(aclrtMallocHost(reinterpret_cast<void **>(&inputBHost), kDataBytes),
                  "aclrtMallocHost(inputB)") ||
        !CheckAcl(aclrtMallocHost(reinterpret_cast<void **>(&outputHost), kDataBytes),
                  "aclrtMallocHost(output)") ||
        !CheckAcl(aclrtMalloc(reinterpret_cast<void **>(&inputADevice), kDataBytes,
                              ACL_MEM_MALLOC_HUGE_FIRST),
                  "aclrtMalloc(inputA)") ||
        !CheckAcl(aclrtMalloc(reinterpret_cast<void **>(&inputBDevice), kDataBytes,
                              ACL_MEM_MALLOC_HUGE_FIRST),
                  "aclrtMalloc(inputB)") ||
    ...
    LaunchMatmul64Kernel(workspace0Device, workspace1Device, inputADevice,

...
KERNEL_MLIR="${KERNEL_MLIR:-${SCRIPT_DIR}/../../out/npuir-ptoas-bridge/vadd_large/vadd_large.vpto.mlir}"
USE_MSPROF="${USE_MSPROF:-0}"
KERNEL_NAME="${KERNEL_NAME:-vector_add_large_kernel}"
CORE_ID="${CORE_ID:-0}"
...
if [[ ! -f "${KERNEL_MLIR}" ]]; then
    ...
    echo "[ERROR]   Planner/bridge/tools/run_npuir_ptoas_bridge_tests.sh --emit-vpto vadd_large" >&2
    exit 1
fi
...
if [[ "${USE_MSPROF}" == "1" ]]; then
    ...
    msprof op simulator \
        ...
        --application="${BUILD_DIR}/lowered_vector_add_large_vpto"
else

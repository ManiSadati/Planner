...
KERNEL_OBJ="${KERNEL_OBJ:-${SCRIPT_DIR}/vector_add_large_kernel_ptoas_fatobj.o}"
...
if [[ ! -f "${KERNEL_OBJ}" ]]; then
    ...
    echo "[ERROR] Copy vector_add_large_kernel_ptoas_fatobj.o here or set KERNEL_OBJ=/path/to/fatobj.o." >&2
    exit 1
fi
...
"${BISHENG_BIN}" \
    ...
    -shared -Wl,-soname,libvector_add_large_kernel.so \
    -L "${ASCEND_HOME_PATH}/lib64" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/libvector_add_large_kernel.so" \
    "${KERNEL_OBJ}" \
    "${BUILD_DIR}/launch.o" \
...
"${BISHENG_BIN}" \
    ...
    -Wl,-rpath,"${BUILD_DIR}" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/lowered_vector_add_large_vpto" \
    -lvector_add_large_kernel \
...
LD_LIBRARY_PATH="${BUILD_DIR}:${ASCEND_HOME_PATH}/lib64:${LD_LIBRARY_PATH:-}" \
    "${BUILD_DIR}/lowered_vector_add_large_vpto"

...
def q_kt_matmul_kernel(
    ...
    for k_block in range(0, tl.cdiv(HEAD_DIM, BLOCK_K)):
        ...
        # KT is physically stored as contiguous [H_KV, HEAD_DIM, SK].
        kt_ptrs = (
            kt_ptr
            + kv_head * HEAD_DIM * SK
            + k_dim[:, None] * SK
...
def main():
    ...
    kt_host = torch.ones((h_kv, head_dim, sk), dtype=torch.float16)
    kt_host[1].fill_(2.0)
    q = q_host.to("npu")
    kt = kt_host.to("npu")
    ...
    q_kt_matmul_kernel[(num_programs,)](
        q,
        kt,
        scores,
        SQ=sq,

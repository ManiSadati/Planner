...
KERNEL_OBJ="${KERNEL_OBJ:-${SCRIPT_DIR}/q_kt_matmul_kernel_ptoas_fatobj.o}"
...
"${BISHENG_BIN}" \
    ...
    -shared -Wl,-soname,libq_kt_matmul_kernel.so \
    -L "${ASCEND_HOME_PATH}/lib64" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/libq_kt_matmul_kernel.so" \
    "${KERNEL_OBJ}" \
    "${BUILD_DIR}/launch.o" \

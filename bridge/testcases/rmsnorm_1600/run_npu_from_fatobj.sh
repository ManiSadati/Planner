...
KERNEL_OBJ="${KERNEL_OBJ:-${SCRIPT_DIR}/rmsnorm_kernel.o}"
...
if [[ ! -f "${KERNEL_OBJ}" ]]; then
    ...
    echo "[ERROR] Copy rmsnorm_kernel.o here or set KERNEL_OBJ=/path/to/rmsnorm_kernel.o." >&2
    exit 1
fi
...
"${BISHENG_BIN}" \
    ...
    -shared -Wl,-soname,librmsnorm_kernel.so \
    -L "${ASCEND_HOME_PATH}/lib64" \
    -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
    -o "${BUILD_DIR}/librmsnorm_kernel.so" \
    "${KERNEL_OBJ}" \
    "${BUILD_DIR}/launch.o" \
